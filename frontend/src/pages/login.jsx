...
import {
  Eye, EyeOff, Lock, Mail, Factory, Shield, Cog, Key, ArrowRight,
} from "lucide-react";
...
const Login = () => {
  ...
  const forgotPasswordMutation = useMutation({
    mutationFn: (emailAddr) => api.post("/auth/forgot-password", { email: emailAddr }),
    onSuccess: () => {
      success(
        "Permintaan Diproses",
  ...
  // ✅ FIX: login() sekarang pure async - throw on error, resolve on success
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute inset-0">
          <div className="absolute inset-0 opacity-10" style={{ backgroundImage: `linear-gradient(90deg, #475569 1px, transparent 1px), linear-gradient(180deg, #475569 1px, transparent 1px)`, backgroundSize: "40px 40px" }} />
        </div>
        <div className="absolute inset-0">
      ...
      <div className="absolute inset-0 pointer-events-none">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="absolute w-16 h-16 bg-gradient-to-t from-gray-700/0 via-gray-600/10 to-gray-700/0 rounded-full animate-float" style={{ left: `${15 + i * 10}%`, bottom: "-30px", animationDelay: `${i * 0.5}s`, animationDuration: "8s" }} />
        ))}
      </div>
      <div className="absolute inset-0 pointer-events-none">
        {[...Array(12)].map((_, i) => {
          const icons = [<Cog key="cog" className="w-4 h-4 text-blue-400/20" />, <Factory key="factory" className="w-4 h-4 text-orange-400/20" />, <Shield key="shield" className="w-4 h-4 text-emerald-400/20" />];
          const Icon = icons[i % 3];
          return <div key={i} className="absolute animate-float-slow" style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, animationDelay: `${i * 0.7}s`, animationDuration: "15s" }}>{Icon}</div>;
        })}
      </div>

      {/* Main Login Card */}
      <div className="relative z-10 w-full max-w-md" onMouseEnter={() => setMainCardHovered(true)} onMouseLeave={() => setMainCardHovered(false)}>
        <div className="relative">
          <div className={`absolute -inset-0.5 bg-gradient-to-br from-gray-700 via-gray-800 to-gray-900 rounded-2xl blur opacity-60 transition-all duration-500 ${mainCardHovered ? "opacity-80" : ""}`} />
          ...
          <div className="relative bg-gradient-to-br from-slate-800 via-gray-800 to-slate-900 rounded-2xl border border-gray-700/50 overflow-hidden shadow-2xl animate-slideInIndustrial">
            <div className="absolute inset-0 opacity-10" style={{ backgroundImage: `linear-gradient(45deg, transparent 48%, #4b5563 50%, transparent 52%), linear-gradient(-45deg, transparent 48%, #4b5563 50%, transparent 52%)`, backgroundSize: "20px 20px" }} />
            <div className="relative overflow-hidden">
              ...
              <div className="relative px-8 pt-10 pb-6 text-center">
                  ...
                  <div className="relative w-20 h-20 rounded-full bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-gray-700 flex items-center justify-center shadow-inner">
                    ...
                    <div className="relative z-10">
                      <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-blue-600 to-cyan-500 flex items-center justify-center shadow-lg">
                        <img src="/Favicon/favJRS.webp" alt="JRS Logo" className="w-8 h-8 object-contain" onError={(e) => { e.target.style.display = "none"; const fb = document.createElement("div"); fb.className = "w-full h-full flex items-center justify-center text-white font-bold text-sm"; fb.textContent = "JRS"; e.target.parentNode.appendChild(fb); }} />
                      </div>
                ...
                <h1 className="mt-6 text-2xl font-bold tracking-tight">
                  <span className="bg-gradient-to-r from-blue-400 via-gray-200 to-orange-400 bg-clip-text text-transparent">JAYA RUBBER SEAL</span>
                </h1>
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                  <div className="group">
                    <div className="relative">
                      ...
                      <input type="email" className="relative w-full bg-gray-900/70 border-2 border-gray-700 rounded-lg px-4 py-3.5 pl-11 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 text-gray-200 placeholder-gray-500 transition-all duration-300 font-mono" value={email} onChange={(e) => setEmail(e.target.value)} required autoComplete="email" placeholder="employee@jayarubberseal.com" disabled={isLoading} />
                      <div className="absolute left-3.5 top-1/2 -translate-y-1/2"><Mail className="w-5 h-5 text-gray-500 group-hover:text-blue-400 transition-colors" /></div>
                    </div>
                  </div>
                  <div className="group">
                    <div className="relative">
                      ...
                      <input type={showPassword ? "text" : "password"} className="relative w-full bg-gray-900/70 border-2 border-gray-700 rounded-lg px-4 py-3.5 pl-11 pr-11 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500/30 text-gray-200 placeholder-gray-500 transition-all duration-300 font-mono" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password" placeholder="••••••••" disabled={isLoading} />
                      <div className="absolute left-3.5 top-1/2 -translate-y-1/2"><Lock className="w-5 h-5 text-gray-500 group-hover:text-orange-400 transition-colors" /></div>
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-orange-400 transition-colors p-1 focus:outline-none focus:ring-2 focus:ring-orange-500/30" tabIndex={-1} disabled={isLoading}>
                        {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                      </button>
                    </div>
                  </div>
                  <div className="flex justify-center">
                    <button type="button" onClick={() => setShowForgotPassword(true)} className="flex items-center text-sm text-gray-400 hover:text-blue-400 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/30 rounded group" disabled={isLoading}>
                      <Key className="w-4 h-4 mr-2" /><span>Lupa Password?</span><ArrowRight className="w-3 h-3 ml-1 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </button>
                  </div>
                  <button type="submit" disabled={isLoading} className={`w-full relative overflow-hidden group mt-4 ${isLoading ? "opacity-80 cursor-not-allowed" : ""}`}>
                      ...
                      {isLoading ? (
                        ...
                        <>
                          ...
                          <div className="ml-3 w-6 h-6 rounded border border-gray-500 group-hover:border-blue-400 flex items-center justify-center transition-colors">
                            <svg className="w-3 h-3 text-gray-400 group-hover:text-blue-300 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                          </div>
                        </>
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-center space-x-4 pt-6">
                    ...
                    <div className="flex items-center space-x-2"><div className="w-1.5 h-1.5 rounded-full bg-blue-500" /><div className="w-1.5 h-1.5 rounded-full bg-orange-500" /><div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /></div>
                    <div className="h-px flex-1 bg-gradient-to-r from-transparent via-gray-700 to-transparent" />
                  </div>
                </form>
              </div>
            </div>
            <div className="px-8 pb-6 text-center"><p className="text-xs text-gray-500 tracking-wider">© {new Date().getFullYear()} JAYA RUBBER SEAL</p></div>
      ...
      {showForgotPassword && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) handleCloseModal(); }}>
          <div className="relative w-full max-w-md" onMouseEnter={() => setModalHovered(true)} onMouseLeave={() => setModalHovered(false)}>
              ...
              <div className="relative p-6">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-600 to-cyan-500 flex items-center justify-center"><Key className="w-6 h-6 text-white" /></div>
                    <h2 className="text-xl font-bold text-gray-200">Reset Password</h2>
                  </div>
                  <button onClick={handleCloseModal} className="p-2 hover:bg-gray-700/50 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/30" type="button" disabled={isResetLoading}>
                ...
                <form onSubmit={handleForgotPassword} className="space-y-4">
                  <div className="group">
                    ...
                    <div className="relative">
                      <div className="absolute inset-0 bg-gradient-to-r from-blue-600/10 to-cyan-600/10 rounded-lg blur opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      <input type="email" className="relative w-full bg-gray-900/70 border-2 border-gray-700 rounded-lg px-4 py-3 pl-11 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 text-gray-200 placeholder-gray-500 transition-all duration-300 font-mono" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} required placeholder="employee@jayarubberseal.com" autoComplete="email" autoFocus disabled={isResetLoading} />
                      <div className="absolute left-3.5 top-1/2 -translate-y-1/2"><Mail className="w-5 h-5 text-gray-500 group-hover:text-blue-400 transition-colors" /></div>
                    </div>
                    <p className="mt-2 text-xs text-gray-500">Link reset password akan dikirim ke email ini</p>
                  </div>
                  <button type="submit" disabled={isResetLoading} className={`w-full relative overflow-hidden group mt-6 ${isResetLoading ? "opacity-80 cursor-not-allowed" : ""}`}>
                    <div className="absolute inset-0 bg-gradient-to-b from-gray-700 via-gray-800 to-gray-900 rounded-lg border-2 border-gray-600" />
                    <div className="absolute inset-0 bg-gradient-to-b from-blue-600 via-blue-700 to-blue-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                    ...
                    <div className="relative py-3.5 rounded-lg flex items-center justify-center">
                      {isResetLoading ? (
                        ...
                        <><span className="text-gray-300 font-semibold tracking-wider group-hover:text-white transition-colors">KIRIM LINK RESET</span><ArrowRight className="w-5 h-5 ml-3 text-gray-400 group-hover:text-white transition-colors" /></>
                      )}
                    </div>
                  </button>
                </form>
                <div className="mt-6 pt-6 border-t border-gray-700/50"><p className="text-xs text-gray-500 text-center">Pastikan email yang Anda masukkan benar</p></div>
              </div>
            </div>
          </div>
        </div>
