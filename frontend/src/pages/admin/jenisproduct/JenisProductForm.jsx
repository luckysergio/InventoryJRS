...
import { useCreateJenisProduct, useUpdateJenisProduct } from "../../../hooks/useJenisProducts";

const JenisProductForm = () => {
  const { isFormOpen, selectedJenis, closeModals } = useJenisProductStore();
  ...
  }, [isFormOpen, isEdit, selectedJenis]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[A-Z0-9\s]+$/.test(trimmedNama)) {
      setError("Nama harus menggunakan HURUF KAPITAL dan boleh mengandung ANGKA");
      return;
    }

    try {
      if (isEdit) {
        await updateMutation.mutateAsync({ id: selectedJenis.id, data: { nama: trimmedNama } });
      } else {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div>
            ...
            {error ? (
              ...
              <p className="mt-1.5 text-xs text-slate-500">Hanya huruf kapital dan angka yang diperbolehkan.</p>
            )}
          </div>
