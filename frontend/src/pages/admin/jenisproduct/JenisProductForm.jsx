...
import { useJenisProductModals } from "../../../lib/zustand/jenisProductStore";
import {
  useCreateJenisProduct,
  useUpdateJenisProduct,
...
const JenisProductForm = () => {
  const { modals, selectedJenis, closeAllModals } = useJenisProductModals();
  const createMutation = useCreateJenisProduct();
  const updateMutation = useUpdateJenisProduct();
  ...
  const isEdit = modals.edit && selectedJenis;
  ...
  useEffect(() => {
    if (isEdit && selectedJenis) {
      setNama(selectedJenis.nama || "");
      setErrors({});
      setTouched({});
    } else if (isCreate) {
      setNama("");
  ...
  }, [isEdit, isCreate, selectedJenis, modals.edit, modals.create]);
  ...
  const validate = (fieldName, value) => {
    ...
    if (fieldName === "nama" || !fieldName) {
      ...
      if (!trimmed) {
        newErrors.nama = "Nama jenis wajib diisi";
        ...
        newErrors.nama = "Hanya huruf kapital, angka, spasi, dan strip";
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        ...
        await success(
          "Berhasil!",
          `Jenis "${trimmedNama}" berhasil diperbarui`
        );
      } else {
        ...
        await success(
          "Berhasil!",
          `Jenis "${trimmedNama}" berhasil ditambahkan`
        );
      }
    } catch (err) {
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-slate-900">
              {isEdit ? "Edit Jenis Product" : "Tambah Jenis Product Baru"}
            </h2>
          </div>
          <button
            ...
            <div className="mt-2 flex items-center justify-between text-xs">
              {errors.nama && touched.nama ? (
                ...
                <p className="text-slate-500">
                  Huruf kapital, angka, spasi, strip. Contoh: KARET ALAM
                </p>
              )}
              <span
                className={cn(
...
export default JenisProductForm;