    ...
    >
      ...
      <div className={cn(
        ...
        <button
          ...
          title={isProtected
            ? `Tidak dapat dihapus (${productsCount} produk, ${typesCount} tipe aktif)`
            : "Hapus Jenis"
          }
        >
          <div className={cn(
...
const JenisProductPage = () => {
  ...
  const { data, isLoading, isFetching, isPlaceholderData, refetch } = useJenisProducts(queryParams);
  ...
  const handleDelete = async (jenis) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || "Gagal menghapus jenis product";
      ...
      <div key={i} className="bg-white border-2 border-slate-200 rounded-2xl overflow-hidden animate-pulse">
        ...
        <div className="p-4 sm:p-5">
          <div className="space-y-2 mb-4">
            ...
            <div className="h-5 w-20 bg-slate-200 rounded-full mt-2" />
          </div>
...
const EmptyState = ({ isFilterActive, onReset, onCreate }) => (
  <div className="bg-white rounded-2xl border border-slate-200/60 p-8 sm:p-12 text-center shadow-sm">
    ...
    <p className="text-slate-900 font-bold text-lg">
      {isFilterActive ? "Tidak ada jenis product yang cocok" : "Belum ada data jenis product"}
    </p>
