    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-modalIn ring-1 ring-black/5">
        ...
        <div className={cn("px-6 py-4 border-b border-slate-200 flex items-center justify-between", config.color.replace('hover:', ''))}>
          ...
          <button onClick={closeAllModals} className="p-2 hover:bg-white/20 rounded-lg transition-colors text-white" disabled={isSubmitting}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="number" min="1"
              value={form.qty}
              onChange={(e) => { setForm({ ...form, qty: e.target.value }); if (errors.qty) setErrors({ ...errors, qty: undefined }); }}
              className={cn("w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 text-sm font-medium", errors.qty ? "border-red-300 focus:ring-red-500" : "border-slate-200 focus:ring-indigo-500")}
          ...
          {movementType === "transfer" && (
            <div>
              ...
              <select
                ...
                onChange={(e) => { setForm({ ...form, to_place_id: e.target.value }); if (errors.to_place_id) setErrors({ ...errors, to_place_id: undefined }); }}
                className={cn("w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 text-sm bg-white", errors.to_place_id ? "border-red-300 focus:ring-red-500" : "border-slate-200 focus:ring-indigo-500")}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Opsional" disabled={isSubmitting}
            />
          </div>
