...
import {
  ...
  ArrowRightLeft, SortAsc, ChevronDown, Lock,
} from "lucide-react";
...
import { useIsAdmin, useIsAdminToko } from "../../../lib/zustand/authStore";
...
const InventoryCard = ({ item, isAdmin, isAdminToko, onMovement }) => {
  ...
  // ✅ LOGIKA AKSES
  const canModifyStock = isAdmin || isAdminToko; // Masuk & Transfer
  const canStockOut = isAdmin;                  // Keluar hanya admin
      ...
      {canModifyStock ? (
        <div className="grid grid-cols-3 border-t-2 border-slate-100 bg-gradient-to-b from-slate-50/50 to-white">
          ...
          {canStockOut ? (
            ...
            // ✅ ADMIN TOKO: Tombol Keluar Disabled dengan Tooltip
            <div
              className="relative group/lock flex flex-col items-center justify-center gap-1 py-3 sm:py-3.5 px-2 cursor-not-allowed"
              title="Hanya admin utama yang dapat mengeluarkan stok"
            >
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-slate-100 flex items-center justify-center transition-all duration-200">
                <Lock size={16} className="text-slate-400" strokeWidth={2.5} />
              </div>
              <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wide">
                Keluar
              </span>
              {/* Tooltip */}
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1.5 bg-slate-900 text-white text-[10px] font-medium rounded-lg shadow-xl opacity-0 group-hover/lock:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
                Hanya admin utama
        ...
        // ✅ USER BIASA: Pesan readonly
        <div className="px-4 py-3 bg-slate-50 border-t border-slate-100 text-center flex items-center justify-center gap-2">
...
const InventoryPage = () => {
  ...
  const isAdminToko = useIsAdminToko();
