...
import {
  ...
  ChevronLeft, ChevronRight, Phone, Mail, Package, Eye, ShieldAlert,
} from "lucide-react";
    ...
    <div className="group relative bg-white border border-slate-200 rounded-xl p-5 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-300">
      <div className="flex flex-col items-center text-center">
        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-slate-50 to-slate-100 group-hover:from-blue-50 group-hover:to-indigo-50 flex items-center justify-center mb-3 transition-all duration-300 ring-2 ring-slate-100 group-hover:ring-blue-200">
          <User className="w-7 h-7 text-slate-400 group-hover:text-blue-600 transition-colors duration-300" />
        </div>
        ...
        <div className="flex flex-col items-center gap-1.5 w-full">
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 rounded-full w-full justify-center">
            <Phone className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
            <span className="text-xs font-medium text-slate-600 truncate">{item.no_hp || "-"}</span>
          </div>

          {item.email && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 rounded-full w-full justify-center">
              <Mail className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
              <span className="text-xs font-medium text-slate-600 truncate">{item.email}</span>
            </div>
          )}
      ...
      <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        ...
        <button onClick={onDelete} disabled={hasProducts} className={cn("p-2 rounded-lg transition-colors", hasProducts ? "text-slate-300 cursor-not-allowed" : "text-slate-500 hover:text-red-600 hover:bg-red-50")} title={hasProducts ? "Tidak dapat dihapus (masih memiliki produk)" : "Hapus"}><Trash2 size={14} /></button>
      </div>
      ...
      {hasProducts && (
        <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] rounded-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none flex items-center justify-center">
          <div className="bg-white px-3 py-1.5 rounded-lg shadow-lg border border-slate-200 flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-slate-700">Memiliki {item.products_count} produk</span>
          </div>
        </div>
      )}
    </div>
  ...
  <div className="bg-white rounded-2xl border border-slate-200/60 p-12 text-center shadow-sm">
    <div className="flex flex-col items-center gap-4">
      <div className="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center">
        {isFilterActive ? <X className="w-10 h-10 text-slate-400" /> : <User className="w-10 h-10 text-slate-400" />}
      </div>
      <div>
        <p className="text-slate-900 font-semibold text-lg">{isFilterActive ? "Tidak ada distributor yang cocok" : "Belum ada data distributor"}</p>
        <p className="text-sm text-slate-500 mt-2 max-w-md">
          {isFilterActive ? "Coba ubah kata kunci pencarian atau reset filter" : "Mulai dengan menambahkan distributor baru untuk mengelola supplier produk Anda"}
        </p>
        {isFilterActive ? (
          ...
          <button onClick={onCreate} className="mt-4 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors inline-flex items-center gap-2"><Plus className="w-4 h-4" /> Tambah Distributor Pertama</button>
        )}
      </div>
    </div>
  </div>
