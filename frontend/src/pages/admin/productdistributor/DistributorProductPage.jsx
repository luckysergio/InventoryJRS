...
import {
  ...
  ChevronLeft, ChevronRight, Tag, Image as ImageIcon,
  Eye, Filter,
} from "lucide-react";
...
const DistributorProductPage = () => {
  const { filters, currentPage, setSearch, setJenisFilter, setTypeFilter, setCurrentPage, resetFilters, hasActiveFilters, getQueryParams } = useDistributorProductFilters();
  ...
  const { data: allTypesOptions = [] } = useTypesDropdown(filters.jenisId || null);
  ...
  }, [filters.jenisId, allTypesOptions]);
  ...
  const handleDelete = async (item) => {
    const confirmed = await danger("Hapus Product?", `Apakah Anda yakin ingin menghapus "${item.kode}"? Tindakan ini tidak dapat dibatalkan.`);
    if (!confirmed) return;
    try {
          ...
          {products.length === 0 ? <EmptyState isFilterActive={isFilterActive} onReset={resetFilters} onCreate={openCreateModal} /> : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {products.map((item) => <DistributorProductCard key={item.id} item={item} onDetail={() => openDetailModal(item)} onEdit={() => openEditModal(item)} onDelete={() => handleDelete(item)} />)}
              </div>
              {lastPage > 1 && (
...
// ✅ CARD DISERAGAMKAN - Layout identik dengan ProductCard
    ...
    <div className="group relative bg-white border border-slate-200 rounded-xl overflow-hidden hover:border-blue-300 hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-300 flex flex-col">
      ...
      <div className="px-4 space-y-1 mb-3 text-xs text-slate-600">
        ...
        <div className="flex items-center justify-center gap-1.5 pt-1 border-t border-slate-100">
          ...
          <span className="text-purple-700 truncate font-medium">{item.distributor?.nama || "-"}</span>
        </div>
      </div>

      <div className="flex-1" />

      {/* Action Buttons - Identik dengan ProductCard */}
  ...
  <div className="bg-white rounded-2xl border border-slate-200/60 p-12 text-center shadow-sm">
    <div className="flex flex-col items-center gap-4">
      <div className="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center">
        {isFilterActive ? <X className="w-10 h-10 text-slate-400" /> : <Truck className="w-10 h-10 text-slate-400" />}
      </div>
      <div>
        <p className="text-slate-900 font-semibold text-lg">{isFilterActive ? "Tidak ada product yang cocok" : "Belum ada data product distributor"}</p>
        <p className="text-sm text-slate-500 mt-2 max-w-md">{isFilterActive ? "Coba ubah filter pencarian atau reset filter" : "Mulai dengan menambahkan product distributor baru"}</p>
