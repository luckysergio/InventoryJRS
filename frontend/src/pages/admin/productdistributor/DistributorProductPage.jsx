...
import { Truck, Pencil, Trash2, Plus, Search, X, RefreshCw, ChevronLeft, ChevronRight, Tag, Warehouse, Image as ImageIcon } from "lucide-react";
import { useDistributorProducts, useDeleteDistributorProduct, useJenis, useTypes } from "../../../hooks/useDistributorProducts";
...
const STORAGE_KEY = 'distributor_product_filters';
...
const loadSavedFilters = () => {
  try {
    ...
    if (saved) {
      ...
      return { search: parsed.search || "", jenisFilter: parsed.jenisFilter || "", typeFilter: parsed.typeFilter || "" };
    }
  } catch (err) {
    // Silent fail
  }
...
const DistributorProductPage = () => {
  const { openCreateModal, openEditModal, openDetailModal } = useDistributorProductStore();
  ...
  const { data, isLoading, isError, refetch, isFetching } = useDistributorProducts(
  ...
  const handleDelete = async (item) => {
    const result = await danger("Hapus Product?", `Apakah Anda yakin ingin menghapus "${item.kode}"? Tindakan ini tidak dapat dibatalkan.`);
    if (result) deleteMutation.mutate(item.id);
  };
  ...
  // ✅ Filter types dengan perbandingan Number yang robust (Sama persis dengan ProductPage)
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">Memuat data product distributor...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 pb-20">
      {/* ============================================
          STICKY SEARCH & FILTER BAR (Identik dengan ProductPage)
      ============================================ */}
      ...
      {products.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200/60 p-12 text-center shadow-sm">
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              <p className="text-slate-900 font-medium text-lg">{isFilterActive ? "Tidak ada product yang cocok" : "Belum ada data product distributor"}</p>
            ...
            return (
              <div key={item.id} className="group relative bg-white border border-slate-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-300 flex flex-col">
                ...
                <div className="space-y-1 mb-3 text-xs text-slate-600">
                  ...
                  <div className="flex items-center justify-center gap-1.5 pt-1 border-t border-slate-100"><Truck className="w-3.5 h-3.5 text-purple-500" /><span className="text-purple-700 truncate">{item.distributor?.nama || "-"}</span></div>
                </div>
