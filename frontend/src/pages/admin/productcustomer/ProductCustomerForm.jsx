...
import {
  X, Image as ImageIcon, Camera, User, Loader2, AlertCircle,
        ...
        <div className={cn("px-6 py-4 border-b border-slate-200 flex items-center justify-between flex-shrink-0", isEdit ? "bg-gradient-to-r from-amber-50 to-white" : "bg-gradient-to-r from-blue-50 to-white")}>
          <div className="flex items-center gap-3">
            <div className={cn("p-2 rounded-lg", isEdit ? "bg-amber-100" : "bg-blue-100")}>
              <Truck className={cn("w-5 h-5", isEdit ? "text-amber-600" : "text-blue-600")} />
            </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto flex-1">
          <div>
            ...
            <div className={cn("w-full px-4 py-2.5 border rounded-lg font-mono font-semibold tracking-wider text-center transition-colors", kodePreview !== "—" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-slate-100 border-slate-200 text-slate-400")}>
              {isEdit ? (selectedItem?.kode || kodePreview) : kodePreview}
            </div>
          </div>
              ...
              {isCreatingNewCustomer && (
                <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200 space-y-3 animate-fadeIn">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-200 focus:outline-none"
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 px-3 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition flex items-center justify-center gap-1 disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setErrors((p) => ({ ...p, new_customer: undefined }));
                      }}
                      className="px-3 py-2 bg-slate-100 text-slate-700 text-sm rounded-lg hover:bg-slate-200 transition"
                    >
                      Batal
                    </button>
                  </div>
          ...
          {isEdit && form.customer_id && (
            <div>
              ...
              <div className="w-full px-4 py-2.5 bg-slate-100 border border-slate-200 rounded-lg text-slate-600 text-sm">
                {customersOptions.find((c) => String(c.value) === String(form.customer_id))?.label || "—"}
              </div>
          ...
          <FormField label="Harga Customer" required error={errors.harga} touched={touched.harga}>
            <div className="relative">
              ...
              <input
                type="text"
                inputMode="numeric"
                value={formatRupiahDisplay(unformatRupiah(form.harga))}
                onChange={(e) => setForm((p) => ({ ...p, harga: String(unformatRupiah(e.target.value)) }))}
                className={cn("w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 text-sm font-medium", errors.harga && touched.harga ? "border-red-300 focus:ring-red-500" : "border-slate-200 focus:ring-blue-500")}
                placeholder="0"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField label="Jenis" required error={errors.jenis_id} touched={touched.jenis_id}>
              <select
                value={form.jenis_id}
                onChange={(e) => {
                  setForm((p) => ({ ...p, jenis_id: e.target.value, type_id: "" }));
                  setNewInputs((p) => ({ ...p, jenis: "", type: "" }));
                  setErrors((p) => ({ ...p, jenis_id: undefined }));
                }}
              ...
              {isNewJenis ? (
                ...
                <>
                  <select
                    value={form.type_id}
                    onChange={(e) => {
                      setForm((p) => ({ ...p, type_id: e.target.value }));
                      setNewInputs((p) => ({ ...p, type: "" }));
                      setErrors((p) => ({ ...p, type_id: undefined }));
                    }}
            ...
            <FormField label="Bahan" error={errors.bahan_id} touched={touched.bahan_id}>
              <select
                value={form.bahan_id}
                onChange={(e) => {
                  setForm((p) => ({ ...p, bahan_id: e.target.value }));
                  setNewInputs((p) => ({ ...p, bahan: "" }));
                  setErrors((p) => ({ ...p, bahan_id: undefined }));
                }}
                className="w-full px-3 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-white"
              ...
              {isNewBahan && (
                <input
                  ...
                  onChange={(e) => setNewInputs((p) => ({ ...p, bahan: e.target.value.toUpperCase() }))}
                  disabled={isSubmitting}
                />
              )}
            </FormField>
