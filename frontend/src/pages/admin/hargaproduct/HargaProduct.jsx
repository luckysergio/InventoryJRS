...
const HargaProductPage = () => {
  const { openCreateModal, openEditModal, openDetailModal } = useHargaProductStore();
  ...
  // ✅ Fetch data dengan server-side filtering & pagination
  ...
  const handleDelete = async (harga) => {
    const result = await danger(
      "Hapus Harga?",
      `Apakah Anda yakin ingin menghapus harga untuk "${formatProductName(harga.product)}"?`
    );
    if (result) {
      deleteMutation.mutate(harga.id);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">Memuat data harga...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              <p className="text-slate-900 font-medium text-lg">
                {isFilterActive ? "Tidak ada data harga yang cocok" : "Belum ada data harga"}
              </p>
              <p className="text-sm text-slate-500 mt-1">
                {isFilterActive ? "Coba ubah filter pencarian Anda" : "Klik tombol '+' untuk membuat harga baru"}
              </p>
              {isFilterActive && (
          ...
          {hargaList.map((item) => (
            <div key={item.id} className="group relative bg-white border border-slate-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-300">
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="font-semibold text-slate-900 text-sm leading-tight line-clamp-2 min-h-[2.5rem] flex items-center justify-center mb-2">
                ...
                <div className="flex flex-col items-center gap-1.5 w-full">
                  <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full w-full justify-center text-xs font-medium ${item.customer_id ? 'bg-purple-50 text-purple-700' : 'bg-slate-100 text-slate-700'}`}>
                    {item.customer_id ? <User className="w-3.5 h-3.5" /> : <Globe className="w-3.5 h-3.5" />}
                    <span className="truncate">{item.customer?.name || "Harga Umum"}</span>
                  </div>
      ...
      {lastPage > 1 && (
        <div className="px-6 py-4 bg-white border border-slate-200/60 rounded-xl shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="text-sm text-slate-600 text-center sm:text-left">
            Menampilkan <span className="font-semibold text-slate-900">{from}</span> - <span className="font-semibold text-slate-900">{to}</span> dari <span className="font-semibold text-slate-900">{total}</span> data harga
          </div>
