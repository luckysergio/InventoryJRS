...
import { X, Calendar, Wallet, Package } from "lucide-react";
import { useCustomerModals } from "../../../lib/zustand/customerStore";
import { formatRupiah, formatTanggal, formatProductName, normalizeDetails } from "../transaksidaily/utils/transaksiUtils";
...
const CustomerTagihanModal = () => {
  ...
  // Filter detail tagihan yang belum lunas
  const details = useMemo(() => {
    if (!isOpen || !selectedItem?.transaksi_details) return [];
    
    return normalizeDetails(selectedItem.transaksi_details).filter((d) => {
      ...
      const sisa = Number(d.sisa_tagihan) || Math.max((Number(d.subtotal) || 0) - (Number(d.total_bayar) || 0), 0);
      return sisa > 0;
    });
  }, [isOpen, selectedItem, tagihanFilter]);

  // Hitung total tagihan
  const totalTagihan = details.reduce((sum, d) => {
    const sisa = Number(d.sisa_tagihan) || Math.max((Number(d.subtotal) || 0) - (Number(d.total_bayar) || 0), 0);
    return sum + sisa;
  }, 0);
        ...
        <div className="px-5 sm:px-6 py-3 border-b border-slate-100 bg-slate-50 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-lg font-bold text-slate-900">Rp {formatRupiah(totalTagihan)}</p>
            </div>
