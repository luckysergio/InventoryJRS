...
import { X, User, Phone, Mail, Receipt, Wallet, Calendar, Pencil, AlertTriangle } from "lucide-react";
import { useCustomerModals } from "../../../lib/zustand/customerStore";
import { cn } from "../../../lib/utils";

const formatRupiah = (value) => new Intl.NumberFormat("id-ID").format(Math.round(Number(value) || 0));

const CustomerDetail = () => {
  const { modals, selectedItem, closeAllModals, openEditModal, openTagihanModal } = useCustomerModals();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn" onClick={handleBackdropClick} role="dialog" aria-modal="true">
      <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden animate-modalIn ring-1 ring-black/5 max-h-[85vh] flex flex-col">
          ...
          <div className="px-5 py-4 space-y-2">
            ...
            <div className="mt-4 pt-4 border-t border-slate-200">
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div className={cn("p-3 rounded-xl border text-center", tagihanHarian > 0 ? "bg-orange-50 border-orange-200 cursor-pointer hover:bg-orange-100 transition" : "bg-slate-50 border-slate-100")}
                ...
                <div className={cn("p-3 rounded-xl border text-center", tagihanPesanan > 0 ? "bg-purple-50 border-purple-200 cursor-pointer hover:bg-purple-100 transition" : "bg-slate-50 border-slate-100")}
        ...
        <div className="sticky bottom-0 px-5 py-3.5 border-t border-slate-200/60 bg-white flex gap-2 flex-shrink-0">
          ...
          <button onClick={() => { closeAllModals(); setTimeout(() => openEditModal(selectedItem), 150); }} className="flex-1 px-3 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 rounded-lg transition-all shadow-sm hover:shadow-md active:scale-95 flex items-center justify-center gap-1.5">
