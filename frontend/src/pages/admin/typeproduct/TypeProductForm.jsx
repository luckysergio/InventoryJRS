      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-modalIn ring-1 ring-black/5 max-h-[90vh] flex flex-col">
        <div className={cn("px-6 py-4 border-b border-slate-200 flex items-center justify-between flex-shrink-0", isEdit ? "bg-gradient-to-r from-amber-50 to-white" : "bg-gradient-to-r from-blue-50 to-white")}>
          <div className="flex items-center gap-3">
            <div className={cn("p-2 rounded-lg", isEdit ? "bg-amber-100" : "bg-blue-100")}><Layers className={cn("w-5 h-5", isEdit ? "text-amber-600" : "text-blue-600")} /></div>
            <h2 className="text-lg font-semibold text-slate-900">{isEdit ? "Edit Type Product" : "Tambah Type Product Baru"}</h2>
          </div>
          ...
          <FormField label="Jenis Product" required error={errors.jenis_id} touched={touched.jenis_id}>
            <div className="relative">
              ...
              <select
                ...
                onBlur={handleBlur}
                className={cn("w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 text-sm bg-white appearance-none cursor-pointer", errors.jenis_id && touched.jenis_id ? "border-red-300 focus:ring-red-500" : "border-slate-200 focus:ring-blue-500")}
          ...
          <FormField label="Nama Type" required error={errors.nama} touched={touched.nama}>
            <div className="relative">
              ...
              <input
                ...
                className={cn("w-full pl-10 pr-10 py-2.5 border rounded-lg focus:outline-none focus:ring-2 transition-colors text-sm font-medium tracking-wide", errors.nama && touched.nama ? "border-red-300 focus:ring-red-500" : "border-slate-200 focus:ring-blue-500")}
                placeholder="CONTOH: TIPE A" disabled={isSubmitting} autoFocus maxLength={MAX_LENGTH}
              />
            ...
            <div className="mt-2 flex items-center justify-between text-xs">
              {errors.nama && touched.nama ? (
                <p className="text-red-600 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{errors.nama}</p>
              ) : (
                <p className="text-slate-500">Kapital, angka, spasi, (-, (), #). Contoh: TIPE A</p>
              )}
              <span className={cn("font-medium", charPercentage > 90 ? "text-red-600" : charPercentage > 75 ? "text-amber-600" : "text-slate-500")}>{charCount}/{MAX_LENGTH}</span>
            </div>
            <div className="mt-2 h-1 bg-slate-100 rounded-full overflow-hidden">
              <div className={cn("h-full transition-all duration-300 rounded-full", charPercentage > 90 ? "bg-red-500" : charPercentage > 75 ? "bg-amber-500" : "bg-blue-500")} style={{ width: `${charPercentage}%` }} />
            </div>
          </FormField>
