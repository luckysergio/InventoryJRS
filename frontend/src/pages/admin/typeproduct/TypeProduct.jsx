...
} from "../../../lib/zustand/typeProductStore";
...
const TypeProductPage = () => {
  ...
  const { openCreateModal, openEditModal, openDetailModal } =
    useTypeProductModals();
  const { danger, success, info, warning } = useConfirmDialog();

  const [searchInput, setSearchInput] = useState(filters.search);

  // Ambil data jenis untuk dropdown filter (lightweight)
  ...
  const handleDelete = async (type) => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || "Gagal menghapus type product";
      await info(
    ...
    <div className="group relative bg-white border border-slate-200 rounded-xl p-5 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-300">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <div className="flex flex-col items-center gap-1.5 w-full">
          ...
          <div
            className={cn(
              "flex items-center gap-1.5 px-2.5 py-1 rounded-full w-full justify-center transition-colors",
              hasProducts
      ...
      <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        ...
        <button
          ...
          title={
            isProtected ? "Tidak dapat dihapus (masih digunakan)" : "Hapus"
          }
        >
          <Trash2 size={14} />
        </button>
        ...
        <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] rounded-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none flex items-center justify-center">
          <div className="bg-white px-3 py-1.5 rounded-lg shadow-lg border border-slate-200 flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-slate-700">
              Digunakan {item.products_count} produk
            </span>
          </div>
        </div>
      )}
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div>
        <p className="text-slate-900 font-semibold text-lg">
          {isFilterActive
            ? "Tidak ada type product yang cocok"
            : "Belum ada data type product"}
        </p>
        <p className="text-sm text-slate-500 mt-2 max-w-md">
          {isFilterActive
            ? "Coba ubah filter pencarian Anda atau reset filter untuk melihat semua data"
